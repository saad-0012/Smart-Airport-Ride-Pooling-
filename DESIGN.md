...
Prevent double-booking (e.g., two users booking the last seat simultaneously).
...
```mermaid
graph TD
    A[Booking Request] --> B{Check Base Fare}
    B --> C[Calculate Distance Surcharge]
    C --> D{High Demand?}
    D -->|Yes| E[Apply 1.2x Multiplier]
    D -->|No| F[Standard Pricing]
    E --> G[Add Pooling Discount]
    F --> G
    G --> H[Final Price Calculated]
    H --> I[Return to User]
```
...
sequenceDiagram
    ...
    User ->> API: POST /book_ride
    API ->> DB: Query nearby rides (2km radius)
    DB -->> API: Return matching rides
    API ->> API: Calculate detour distance
    API ->> DB: Atomic find_one_and_update
    DB -->> API: Booking confirmed/failed
    
    alt Booking Successful
        API ->> Cache: Update ride status
        API -->> User: Ride booked (ID, Price, ETA)
    else Double-booked
        API ->> API: Create new ride
        API -->> User: New ride created
    end
```
...
This design ensures:
- ✅ **Scalability:** O(log M) complexity for 10,000+ concurrent users
- ✅ **Low Latency:** <300ms response time
- ✅ **Concurrency Safety:** Atomic database operations prevent race conditions
- ✅ **Cost Efficiency:** Pooling incentives through dynamic pricing
- ✅ **Geospatial Efficiency:** Database-level filtering reduces application overhead